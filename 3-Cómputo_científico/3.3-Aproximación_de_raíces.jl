...
# ╔═╡ fb79d177-0c3d-4240-bf72-f35e7767cf49
using Plots
...
# ╔═╡ defb5c70-1934-431e-b300-e1fc786eb268
md"""$$x_{i+1} = x_i -\frac{f(x_i)}{f'(x_i)}.$$"""
...
# ╔═╡ 0f253248-0d58-4739-9ffb-852f1502535d
function newton(f, fprime, x0, n)
    """
    Aplica el método de Newton para encontrar una raíz de la función f con aproximación inicial x0,
    usando n iteraciones.
    
    Argumentos:
    f -- función
    fprime -- derivada de f
    x0 -- aproximación inicial a la raíz de f
    n -- número de iteraciones
    
    Devuelve:
    La aproximación de la raíz de f después de n iteraciones del método de Newton.
    """
    x = x0
...
#El método de Newton es un algoritmo para encontrar las raíces de una función. Dado un punto inicial x0, el método de Newton utiliza la derivada de la función f para aproximar la pendiente de la recta tangente a la función en x0, y luego busca la intersección de esa recta con el eje x para encontrar una nueva aproximación x1. Luego, se repite este proceso con x1 como punto inicial, y se continúa hasta que se alcance una aproximación suficientemente precisa.
...
function newtonRecursiva(f, fprime, x0, n)
    """
    Aplica el método de Newton para encontrar una raíz de la función f con aproximación inicial x0,
    usando n iteraciones de manera recursiva.
    ...
    if n == 0
        ...
        x = x0 - f(x0) / fprime(x0)
        return newtonRecursiva(f, fprime, x, n-1)
    end

# La función newtonRecursiva es similar a la función newton, pero utiliza una llamada recursiva en lugar de un ciclo for. La función verifica si hemos alcanzado el número máximo de iteraciones (n), y si es así, devuelve la aproximación actual de la raíz. De lo contrario, calcula una nueva aproximación x y llama a la función recursivamente con esta nueva aproximación y el número de iteraciones reducido en 1.
...
function newtonDistancia(f, fprime, x0, epsilon)
    """
    Aplica el método de Newton para encontrar una raíz de la función f con aproximación inicial x0,
    utilizando iteraciones hasta que la distancia entre dos aproximaciones consecutivas sea menor a epsilon.
    
    Argumentos:
    f -- función
    fprime -- derivada de f
    x0 -- aproximación inicial a la raíz de f
    epsilon -- la distancia entre dos aproximaciones consecutivas del método de Newton
    
    Devuelve:
    La aproximación de la raíz de f cuando la distancia entre dos aproximaciones consecutivas del método de Newton sea menor a epsilon.
    """
...
# La función newtonDistancia es similar a la función newton, pero utiliza un ciclo while en lugar de un ciclo for. La función calcula la primera aproximación x1 y luego entra en un ciclo while. En cada iteración del ciclo, calcula una nueva aproximación x y verifica si la distancia entre x y la aproximación anterior x0 es menor que epsilon. Si es así, la función devuelve x. De lo contrario, actualiza x0 a x y calcula una nueva aproximación.
...
function derivadaNumerica(f, x, h)
    """
    Calcula una aproximación de la derivada numérica de f en x con un valor de h dado.
    
    Argumentos:
    f -- función
    x -- punto del dominio de f
    h -- valor grande
    
    Devuelve:
    Una aproximación de la derivada numérica de f en x.
    """
    # Calcula la aproximación de la derivada numérica usando la fórmula (f(x+h) - f(x-h)) / (2*h)
    return (f(x + h) - f(x - h)) / (2 * h)
end


# ╔═╡ f7ef8cd8-4ae4-4ec7-90c5-d689d44cae18
begin
function cuadrado(x)
    return x^2
end

derivada = derivadaNumerica(cuadrado, 2, 0.01)
println(derivada)  # Salida: 3.9999999999995595

#"cuadrado´" es la función que queremos derivar, 2 es el punto en el que queremos calcular la derivada y 0.01 es el valor grande que usamos para la aproximación. La función devuelve una aproximación de la derivada numérica de cuadrado en 2. En este caso, el resultado es 3.9999999999995595.
...
function newtonDN(f, x0, n, h)
    """
    Aplica el método de Newton para encontrar una raíz de la función f con aproximación inicial x0,
    utilizando n iteraciones del método de Newton y una aproximación numérica de la derivada de f con un valor h.
    
    Argumentos:
    f -- función
    x0 -- aproximación inicial a la raíz de f
    n -- número de iteraciones del método de Newton
    h -- valor grande para calcular la aproximación numérica de la derivada de f
...
#La función newtonDN es similar a la función newton, pero en lugar de tomar la derivada de f como un argumento separado, utiliza la función derivadaNumérica para calcular una aproximación numérica de la derivada de f. La función realiza n iteraciones del método de Newton utilizando la derivada numérica de f con un valor h grande.
...
# ╔═╡ 51a8920c-effd-4d8a-969f-0d6989b12be9
@bind x0 Slider(-10:0.1:10, default=0.1)          #Se define un deslizador para x0

# ╔═╡ e49018e0-2112-4e1d-9832-268041d10642
@bind h Slider(1000:1000:1000000, default=1000)       #Se define un deslizador para h

# ╔═╡ 95872021-d53d-4810-a99c-0b49fb41b1fa
@bind n Slider(1:1000)                             #Se define un deslizador para n

# ╔═╡ 005372a4-35cc-4d81-846e-0aa9b66a6ce5
f = cos # Luego, usa esta celda para reasignar `f`

# ╔═╡ cf43e8bb-154c-4b02-a32b-118622b958f7
begin
function f1(x)
    return x^3 - 2x - 5
end
...
raiz = newton(f, fprime, 2, 5)
println(raiz)  # Salida: 2.2216096082454526

# En este ejemplo, f es la función para la cual queremos encontrar una raíz, fprime es su derivada, 2 es una aproximación inicial a la raíz de f, y 5 es el número de iteraciones que queremos hacer. La función devuelve una aproximación de la raíz de f después de 5 iteraciones del método de Newton. En este caso, el resultado es 2.0945514815423265.
	
end

# ╔═╡ b7ec4726-a18d-41ba-a679-81ac0e645530
begin
function f2(x)
    return x^3 - 2x - 5
end
...
raiz2 = newtonRecursiva(f, fprime, 2, 5)
println(raiz)  # Salida: 2.2216096082454526

# En este ejemplo, la función f y su derivada fprime se definen de la misma manera que en el ejemplo anterior. La función newtonRecursiva se llama con una aproximación inicial de 2 y 5 iteraciones. La función devuelve una aproximación de la raíz de f después de 5 iteraciones del método de Newton, que es 2.2216096082454526.
	
end

# ╔═╡ eec68c5e-f4b4-499d-bb5d-8a801b5e2ab3
begin
function f3(x)
    return x^3 - 2x - 5
end
...
raiz3 = newtonDistancia(f, fprime, 2, 1e-6)
println(raiz)  # Salida: 2.2216096082454526

#En este ejemplo, la función f y su derivada fprime se definen de la misma manera que en los ejemplos anteriores. La función newtonDistancia se llama con una aproximación inicial de 2 y una distancia de 1e-6. La función devuelve una aproximación de la raíz de f cuando la distancia entre dos aproximaciones consecutivas del método de Newton sea menor a 1e-6, que es 2.2216096082454526.
	
end

# ╔═╡ fb17a1d3-135a-4a6a-85b8-64c9f60218a0
begin
function f4(x)
    return x^3 - 2x - 5
end

raiz4 = newtonDN(f, 2, 10, 1e-6)
println(raiz)  # Salida: 2.2216096082454526

# En este ejemplo, la función f se define de la misma manera que en los ejemplos anteriores. La función newtonDN se llama con una aproximación inicial de 2, 10 iteraciones del método de Newton y un valor h de 1e-6. La función devuelve una aproximación de la raíz de f después de 10 iteraciones del método de Newton, que es 2.2216096082454526.
...
begin
x_range = -100:0.1:100                             #Definimos el rango de x
xi = [newtonDn(f, x0, h,i) for i in 1:n]         #Definimos xi como el valor de newtonDn
fi = f.(xi)                                      #Definimos f para un xi
scatter(xi, fi, marker=:circle, line=:true, label="", xlabel ="x", ylabel="f(x)", title="Gráfica Newton")   # graficamos cada punto
	plot!(f)
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "95d149a863af50f423541e3078f7567cf36f1609"
