...
# ╔═╡ 9b78552f-39bc-46f5-ad4a-54cda9357638
using PlutoUI
...
# ╔═╡ 12076d34-325e-44e2-b6cc-e4983f2c5b36
rand() #Genera un número aletorio de tipo flotante entre 0 y 1

# ╔═╡ 7191f0cf-7758-479e-aad5-fdd0d4d22479
rand(Int) #Genera números enteros de tipo Int32

# ╔═╡ d954001e-78dd-4896-a0c5-a74a45063b2e
rand(Bool) #Genera valores booleanos (true y false)
...
# ╔═╡ efdb5a59-b0fd-4340-9ab9-11ff8acda83a
aleatorioUniforme(10,5)
...
# ╔═╡ d53a8015-f5ee-4f40-8908-6b2c0dbf0223
begin
# Generar 100000 números aleatorios en el intervalo [0,1]
numeros = [aleatorioUniforme(0, 1) for i in 1:100000]

# Graficar histograma
histogram(numeros, bins = 50, xlabel = "Número aleatorio", ylabel = "Frecuencia", legend = false)
end
...
# ╔═╡ 1a9b3ea8-4ef3-4a9d-9c83-b7756acae244
unPaso()
...
function variosPasos(n)
    return rand(Bool, n) .* 2 .- 1
end

# ╔═╡ c93de5db-093b-4a85-86f2-2185f00ec5af
variosPasos(10)
...
function caminataAleatoria(n)
    pasos = variosPasos(n)
    caminata = cumsum([0; pasos])
    return caminata
end

# ╔═╡ d9a8e77b-0681-4baa-8d95-a79f53a97837
caminataAleatoria(5)
...
# ╔═╡ 26f31af3-549f-4080-8146-431412d7206e
function graficaCaminata(caminata)
    n = length(caminata) - 1
    scatter(0:n, caminata, xlabel="Número de pasos", ylabel="Posición")
end

# ╔═╡ 64639791-26ca-4c98-8b43-2703cdf8c23e
begin
n2 = 100
...
# ╔═╡ 52d998c8-59e1-4efe-9aba-a0e0073802b9
function graficaCaminata!(caminataleatoria)                      
	n = length(caminataleatoria)                                 
...
# ╔═╡ 866fd1c7-88db-437a-b9d6-df2ce1bb5924
rand(Int,n)

# ╔═╡ bafe625a-90ab-4890-8697-776046adb0cd
rand(Bool,n)
...
# ╔═╡ 7dc202d2-28f1-4e36-9927-e3d3e7d82b98
animaCaminata(caminata)
...
# ╔═╡ de30c62f-9ed5-4bab-aff7-43018b050bba
begin
function caminataAleatoria2D(n)                                           
	...
	for i in 2:n+1                                                       
		dx, dy = aleatorioUniforme(-1,1), aleatorioUniforme(-1,1)        
...
# ╔═╡ 55fde005-74ee-4cc4-8ba0-9b051d641b4f
begin
function graficaCaminata2D(camino)                                        
x = [p[1] for p in camino]                                                
	y = [p[2] for p in camino]                                            
plot(x, y, linestyle =:dash, aspect_radio = 1, title= "Gráfica caminatas aleatorias en dos dimensiones")  
...
# ╔═╡ 25d58a37-4d01-4602-bf28-1da85720f67c
begin
function animaCaminata2D(caminatal)                               
	animacion1 = plot(title="Caminata aleatoria en 2d", xlims=(-50,50), ylims= (-50,50))   #
		for i in 1:length(caminatal)
		graficaCaminata2D(caminatal[1:i])   
	end
	return animacion1                       
end

function graficaCaminata2D(caminatal, animacion1)        
	x = cumsum(caminatal[1,:])                           
	y = cumsum(caminatal[2,:])                            
	plot!(animacion1, x, y, color=:blue, lw=1.5)          
	scatter!(animacion1, x[end], y[end], color=:green, ms=4)    
end
	caminatal1 = [randn(2) for i in 1:25]                
	@gif for i in 1:length(caminatal1)                   
		animaCaminata2D(caminatal1[1:i])                  
end
end
...
function caminataAleatoria3D(n)
    ...
    for i in 1:n
        paso = [aleatorioUniforme(-1, 1), aleatorioUniforme(-1, 1), aleatorioUniforme(-1, 1)]
        posiciones[1] = push!(posiciones[1], posiciones[1][end] + paso[1])
        posiciones[2] = push!(posiciones[2], posiciones[2][end] + paso[2])
        posiciones[3] = push!(posiciones[3], posiciones[3][end] + paso[3])
    end
    return posiciones
end


# ╔═╡ af566388-6823-4bd0-b2a1-769cb6dde7e7

function graficaCaminata3D(posiciones)
    plot(posiciones[1], posiciones[2], posiciones[3], 
         marker=:circle, markersize=3, linestyle=:dot, label="")
    xlabel!("x")
    ylabel!("y")
    zlabel!("z")
end
...
# ╔═╡ 442c97a0-7f5e-46dd-9b77-fed014745f47
begin
n5 = 1000
trayectoria = caminataAleatoria3D(n5)
graficaCaminata3D(trayectoria)
end

# ╔═╡ 49a1e0cb-06ac-4409-9ed6-ceaa18bf483c
begin
function graficaCaminata2D!(Caminata,g)                       
	x = [p[1] for p in Caminata]                           
	y = [p[2] for p in Caminata]                              
	plot!(g, x, y, linestyle=:dash, aspect_ratio=1)           
    display(g)                     
end
	n6= 9876                                                  
g = plot(title= "Cinco caminatas aleatorias en dos dimensiones espaciales", xlabel= "Coordenada x", ylabel = "Coordenada y", legend = false, aspect_ratio=1)                                 
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "f0af775958fb5190eb971c3f4b910f5c7d5004c9"
